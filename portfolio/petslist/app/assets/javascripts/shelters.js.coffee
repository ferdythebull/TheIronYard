...
load_on_page = ->
  if $("#map").length > 0
    ...
    map = L.mapbox.map("map", "EVENTUAL_LINK_GOES_HERE").setView([
      COORDINATE1
      COORDINATE2
    ], 12)
    listings = document.getElementById("listings")
    locations = L.mapbox.featureLayer().addTo(map)
    locations.loadURL "shelters.json"
    locations.on "ready", ->
      locations.eachLayer (locale) ->
        ...
        popup = "<h3>" + "<a href='http://localhost:3000/shelters/" + prop.id + "'>" + prop.name + "</a>" + "</h3><div>" + prop.address
    ...
    locations.on "layeradd", (e) ->
      ...
      marker.setIcon L.icon(
        iconUrl: "EVENTUAL_TEST_LINK_GOES_HERE"
        iconSize: [
          48
          48
        ]
